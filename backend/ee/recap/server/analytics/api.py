...
from ee.recap.db.analytics import fetch_assistant_message_analytics
from ee.recap.db.analytics import fetch_assistant_unique_users
from ee.recap.db.analytics import fetch_assistant_unique_users_total
from ee.recap.db.analytics import fetch_recapbot_analytics
from ee.recap.db.analytics import fetch_per_user_query_analytics
from ee.recap.db.analytics import fetch_persona_message_analytics
from ee.recap.db.analytics import fetch_persona_unique_users
from ee.recap.db.analytics import fetch_query_analytics
from ee.recap.db.analytics import user_can_view_assistant_stats
...
class RecapbotAnalyticsResponse(BaseModel):
    total_queries: int
    auto_resolved: int
    date: datetime.date


@router.get("/admin/recapbot")
def get_recapbot_analytics(
    start: datetime.datetime | None = None,
    end: datetime.datetime | None = None,
    _: User | None = Depends(current_admin_user),
    db_session: Session = Depends(get_session),
) -> list[RecapbotAnalyticsResponse]:
    daily_recapbot_info = fetch_recapbot_analytics(
        start=start
        or (
            datetime.datetime.utcnow() - datetime.timedelta(days=_DEFAULT_LOOKBACK_DAYS)
        ),  # default is 30d lookback
    ...
    resolution_results = [
        ...
        for total_queries, total_negatives, date in daily_recapbot_info
    ]
